...
const css = `
  ...
  .el-add-fab {
    width:44px; height:44px; border-radius:14px; background:#e94560; border:none; color:white;
    font-size:20px; display:flex; align-items:center; justify-content:center;
    cursor:pointer; box-shadow:0 4px 14px rgba(233,69,96,0.45);
    -webkit-tap-highlight-color:transparent; flex-shrink:0; transition:transform 0.15s;
  }
  .el-add-fab:active { transform:scale(0.92); }

  .el-stats {
    display:grid; grid-template-columns:repeat(4,1fr); gap:0;
    margin:-14px 16px 0; background:white; border-radius:16px; overflow:hidden;
...
export default function ElectricityPage() {
  ...
  // Rent collected = payment marked isCompleted for that month
  ...
  const handleAddBill = async () => {
    ...
    try {
      await addDoc(collection(db,'electricityBills'),{
        roomNumber:form.roomNumber, amount:parseInt(form.amount),
        month:form.month, year:form.year, notes:form.notes,
        readingDate:form.readingDate, tenantCount:rt.length,
        tenantIds:rt.map(t=>t.id), tenantNames:rt.map(t=>t.name),
        isPaid:false, paidTenantIds:[], ownerId:user.uid,
  ...
  const collectedCount   = thisMonthBills.reduce((a,b)=>{
    return a + (b.tenantIds||[]).filter(tid=>isRentCollected(tid,b.month,b.year)).length;
  },0);
                            ...
                            <div key={tenant.id} className="el-tenant-row" style={{
                              ...
                              <div className="el-tr-left">
                                ...
                                <div>
                                  ...
                                  <div className="el-tr-bd">
                                    🏠 ₹{(tenant.monthlyRent||0).toLocaleString()} + ⚡ ₹{perHead.toLocaleString()}
                                  </div>
                                </div>
                              </div>
